...
- Song composer: the single-song counterpart, sitting right after the song inputs — drafts the four inputs from a one-line description, or revises the current song from short notes ("punchier chorus, male vocals"), writing straight into the globals that Generate, the seed sweep, and the web UI read.
